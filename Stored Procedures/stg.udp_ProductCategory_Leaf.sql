...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_ProductCategory_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[ProductCategory_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[ProductCategory_Leaf] stgl  
...
-- Create a new batch ID.  
    ...
    IF @GetNewBatch_ID = @BatchIDNotFound BEGIN  
        UPDATE [stg].[ProductCategory_Leaf]  
...
WHERE Code in (SELECT Code FROM [stg].[ProductCategory_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[ProductCategory_Leaf] stgl  
    INNER JOIN mdm.[tbl_3_25_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[ProductGroup]', inserted.[ProductGroup], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[ProductCategory_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_26_EN dm   
    ON stgl.[ProductGroup] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[ProductGroup],  @NULLText), N'')) > 0   
...
FROM [stg].[ProductCategory_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_3_25_EN] en   
...
FROM [stg].[ProductCategory_Leaf] sc  
    INNER JOIN mdm.[tbl_3_25_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[ProductCategory_Leaf] sc  
    INNER JOIN mdm.[tbl_3_25_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[ProductCategory_Leaf]  
...
(  
            ...
            ,uda_25_715 NVARCHAR(MAX) NULL   
            ,New_uda_25_715 NVARCHAR(MAX) NULL   
);  
...
,[uda_25_715] --[ProductGroup]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[ProductGroup] = @NULLText THEN NULL ELSE [ProductGroup].ID END  
    FROM [stg].[ProductCategory_Leaf] stgl   
LEFT JOIN mdm.tbl_3_26_EN [ProductGroup] ON stgl.[ProductGroup] = [ProductGroup].Code AND  [ProductGroup].Version_ID = @Version_ID  
...
,[uda_25_715] --[ProductGroup]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_25_715])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_3_25_EN] en INNER JOIN [stg].[ProductCategory_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[ProductCategory_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_3_25_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_25_715] = CASE WHEN stgl.[ProductGroup] = @NULLText THEN NULL   
            ELSE ISNULL([ProductGroup].ID, en.[uda_25_715]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_25_715]) , CONVERT(NVARCHAR(MAX), inserted.[uda_25_715])   
        INTO #TRANLOG  
    FROM mdm.[tbl_3_25_EN] en INNER JOIN [stg].[ProductCategory_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[ProductCategory_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[ProductCategory_Leaf]   
