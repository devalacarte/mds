...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_Product_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[Product_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[Product_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[Product_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[ProductSubCategory]', inserted.[ProductSubCategory], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_28_EN dm   
    ON stgl.[ProductSubCategory] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[ProductSubCategory],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Color]', inserted.[Color], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_21_EN dm   
    ON stgl.[Color] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[Color],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Class]', inserted.[Class], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_20_EN dm   
    ON stgl.[Class] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[Class],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Style]', inserted.[Style], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_30_EN dm   
    ON stgl.[Style] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[Style],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Country]', inserted.[Country], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_22_EN dm   
    ON stgl.[Country] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[Country],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[SizeUoM]', inserted.[SizeUoM], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_31_EN dm   
    ON stgl.[SizeUoM] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[SizeUoM],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[WeightUoM]', inserted.[WeightUoM], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_31_EN dm   
    ON stgl.[WeightUoM] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[WeightUoM],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[InHouseManufacture]', inserted.[InHouseManufacture], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_32_EN dm   
    ON stgl.[InHouseManufacture] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[InHouseManufacture],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[FinishedGoodIndicator]', inserted.[FinishedGoodIndicator], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_32_EN dm   
    ON stgl.[FinishedGoodIndicator] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[FinishedGoodIndicator],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[DiscontinuedItemInd]', inserted.[DiscontinuedItemInd], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_32_EN dm   
    ON stgl.[DiscontinuedItemInd] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[DiscontinuedItemInd],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[ProductLine]', inserted.[ProductLine], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_27_EN dm   
    ON stgl.[ProductLine] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[ProductLine],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[DealerCostCurrencyCode]', inserted.[DealerCostCurrencyCode], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_23_EN dm   
    ON stgl.[DealerCostCurrencyCode] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[DealerCostCurrencyCode],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[MSRPCurrencyCode]', inserted.[MSRPCurrencyCode], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_23_EN dm   
    ON stgl.[MSRPCurrencyCode] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[MSRPCurrencyCode],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Size]', inserted.[Size], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_29_EN dm   
    ON stgl.[Size] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[Size],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
...
FROM [stg].[Product_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_3_24_EN] en   
...
FROM [stg].[Product_Leaf] sc  
    INNER JOIN mdm.[tbl_3_24_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Product_Leaf] sc  
    INNER JOIN mdm.[tbl_3_24_EN] AS tSource ON sc.NewCode = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Product_Leaf] sc  
    INNER JOIN mdm.[tbl_3_24_HP] AS tSource ON sc.Code = tSource.Code  
WHERE tSource.Version_ID = @Version_ID AND sc.ImportType IN (@IT_Insert, @IT_MergeOptimistic, @IT_MergeOverwrite) AND sc.ImportStatus_ID = @StatusDefault AND sc.Batch_ID = @Batch_ID;  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Product_Leaf] sc  
    INNER JOIN mdm.[tbl_3_24_HP] AS tSource ON sc.NewCode = tSource.Code  
WHERE tSource.Version_ID = @Version_ID AND sc.ImportType IN (@IT_MergeOptimistic, @IT_MergeOverwrite)  
    AND sc.ImportStatus_ID = @StatusDefault AND sc.Batch_ID = @Batch_ID;  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Product_Leaf] sc  
    INNER JOIN mdm.[tbl_3_24_CN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Product_Leaf] sc  
    INNER JOIN mdm.[tbl_3_24_CN] AS tSource ON sc.NewCode = tSource.Code  
WHERE tSource.Version_ID = @Version_ID AND sc.ImportType IN (@IT_MergeOptimistic, @IT_MergeOverwrite)  
    AND sc.ImportStatus_ID = @StatusDefault AND sc.Batch_ID = @Batch_ID;  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[Product_Leaf]  
...
(  
            ...
            ,uda_24_690 NVARCHAR(MAX) NULL   
            ,uda_24_691 NVARCHAR(MAX) NULL   
            ,uda_24_692 NVARCHAR(MAX) NULL   
            ,uda_24_693 NVARCHAR(MAX) NULL   
            ,uda_24_694 NVARCHAR(MAX) NULL   
            ,uda_24_695 NVARCHAR(MAX) NULL   
            ,uda_24_696 NVARCHAR(MAX) NULL   
            ,uda_24_697 NVARCHAR(MAX) NULL   
            ,uda_24_698 NVARCHAR(MAX) NULL   
            ,uda_24_699 NVARCHAR(MAX) NULL   
            ,uda_24_700 NVARCHAR(MAX) NULL   
            ,uda_24_701 NVARCHAR(MAX) NULL   
            ,uda_24_702 NVARCHAR(MAX) NULL   
            ,uda_24_703 NVARCHAR(MAX) NULL   
            ,uda_24_704 NVARCHAR(MAX) NULL   
            ,uda_24_705 NVARCHAR(MAX) NULL   
            ,uda_24_706 NVARCHAR(MAX) NULL   
            ,uda_24_707 NVARCHAR(MAX) NULL   
            ,uda_24_708 NVARCHAR(MAX) NULL   
            ,uda_24_709 NVARCHAR(MAX) NULL   
            ,uda_24_710 NVARCHAR(MAX) NULL   
            ,uda_24_711 NVARCHAR(MAX) NULL   
            ,uda_24_712 NVARCHAR(MAX) NULL   
            ,uda_24_713 NVARCHAR(MAX) NULL   
            ,uda_24_714 NVARCHAR(MAX) NULL   
            ,New_uda_24_690 NVARCHAR(MAX) NULL   
            ,New_uda_24_691 NVARCHAR(MAX) NULL   
            ,New_uda_24_692 NVARCHAR(MAX) NULL   
            ,New_uda_24_693 NVARCHAR(MAX) NULL   
            ,New_uda_24_694 NVARCHAR(MAX) NULL   
            ,New_uda_24_695 NVARCHAR(MAX) NULL   
            ,New_uda_24_696 NVARCHAR(MAX) NULL   
            ,New_uda_24_697 NVARCHAR(MAX) NULL   
            ,New_uda_24_698 NVARCHAR(MAX) NULL   
            ,New_uda_24_699 NVARCHAR(MAX) NULL   
            ,New_uda_24_700 NVARCHAR(MAX) NULL   
            ,New_uda_24_701 NVARCHAR(MAX) NULL   
            ,New_uda_24_702 NVARCHAR(MAX) NULL   
            ,New_uda_24_703 NVARCHAR(MAX) NULL   
            ,New_uda_24_704 NVARCHAR(MAX) NULL   
            ,New_uda_24_705 NVARCHAR(MAX) NULL   
            ,New_uda_24_706 NVARCHAR(MAX) NULL   
            ,New_uda_24_707 NVARCHAR(MAX) NULL   
            ,New_uda_24_708 NVARCHAR(MAX) NULL   
            ,New_uda_24_709 NVARCHAR(MAX) NULL   
            ,New_uda_24_710 NVARCHAR(MAX) NULL   
            ,New_uda_24_711 NVARCHAR(MAX) NULL   
            ,New_uda_24_712 NVARCHAR(MAX) NULL   
            ,New_uda_24_713 NVARCHAR(MAX) NULL   
            ,New_uda_24_714 NVARCHAR(MAX) NULL   
);  
...
BEGIN  
    ...
    (  
        ...
        LastChgUserID,  
        LastChgVersionID  
,[uda_24_695] --[StandardCost]  
,[uda_24_696] --[SafetyStockLevel]  
,[uda_24_697] --[ReorderPoint]  
,[uda_24_698] --[MSRP]  
,[uda_24_699] --[Weight]  
,[uda_24_700] --[DaysToManufacture]  
,[uda_24_701] --[DealerCost]  
,[uda_24_702] --[DocumentationURL]  
,[uda_24_703] --[SellStartDate]  
,[uda_24_704] --[SellEndDate]  
,[uda_24_710] --[DiscontiuedDate]     
,[uda_24_690] --[ProductSubCategory]     
,[uda_24_691] --[Color]     
,[uda_24_692] --[Class]     
,[uda_24_693] --[Style]     
,[uda_24_694] --[Country]     
,[uda_24_705] --[SizeUoM]     
,[uda_24_706] --[WeightUoM]     
,[uda_24_707] --[InHouseManufacture]     
,[uda_24_708] --[FinishedGoodIndicator]     
,[uda_24_709] --[DiscontinuedItemInd]     
,[uda_24_711] --[ProductLine]     
,[uda_24_712] --[DealerCostCurrencyCode]     
,[uda_24_713] --[MSRPCurrencyCode]     
,[uda_24_714] --[Size]  
    )  
    SELECT  
        @Version_ID,  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,NULLIF(stgl.[StandardCost],@NULLNumber),  
NULLIF(stgl.[SafetyStockLevel],@NULLNumber),  
NULLIF(stgl.[ReorderPoint],@NULLNumber),  
NULLIF(stgl.[MSRP],@NULLNumber),  
NULLIF(stgl.[Weight],@NULLNumber),  
NULLIF(stgl.[DaysToManufacture],@NULLNumber),  
NULLIF(stgl.[DealerCost],@NULLNumber),  
NULLIF(stgl.[DocumentationURL],@NULLText),  
NULLIF(stgl.[SellStartDate],@NULLDateTime),  
NULLIF(stgl.[SellEndDate],@NULLDateTime),  
NULLIF(stgl.[DiscontiuedDate],@NULLDateTime),CASE WHEN stgl.[ProductSubCategory] = @NULLText THEN NULL ELSE [ProductSubCategory].ID END,  
CASE WHEN stgl.[Color] = @NULLText THEN NULL ELSE [Color].ID END,  
CASE WHEN stgl.[Class] = @NULLText THEN NULL ELSE [Class].ID END,  
CASE WHEN stgl.[Style] = @NULLText THEN NULL ELSE [Style].ID END,  
CASE WHEN stgl.[Country] = @NULLText THEN NULL ELSE [Country].ID END,  
CASE WHEN stgl.[SizeUoM] = @NULLText THEN NULL ELSE [SizeUoM].ID END,  
CASE WHEN stgl.[WeightUoM] = @NULLText THEN NULL ELSE [WeightUoM].ID END,  
CASE WHEN stgl.[InHouseManufacture] = @NULLText THEN NULL ELSE [InHouseManufacture].ID END,  
CASE WHEN stgl.[FinishedGoodIndicator] = @NULLText THEN NULL ELSE [FinishedGoodIndicator].ID END,  
CASE WHEN stgl.[DiscontinuedItemInd] = @NULLText THEN NULL ELSE [DiscontinuedItemInd].ID END,  
CASE WHEN stgl.[ProductLine] = @NULLText THEN NULL ELSE [ProductLine].ID END,  
CASE WHEN stgl.[DealerCostCurrencyCode] = @NULLText THEN NULL ELSE [DealerCostCurrencyCode].ID END,  
CASE WHEN stgl.[MSRPCurrencyCode] = @NULLText THEN NULL ELSE [MSRPCurrencyCode].ID END,  
CASE WHEN stgl.[Size] = @NULLText THEN NULL ELSE [Size].ID END  
    FROM [stg].[Product_Leaf] stgl   
LEFT JOIN mdm.tbl_3_28_EN [ProductSubCategory] ON stgl.[ProductSubCategory] = [ProductSubCategory].Code AND  [ProductSubCategory].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_21_EN [Color] ON stgl.[Color] = [Color].Code AND  [Color].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_20_EN [Class] ON stgl.[Class] = [Class].Code AND  [Class].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_30_EN [Style] ON stgl.[Style] = [Style].Code AND  [Style].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_22_EN [Country] ON stgl.[Country] = [Country].Code AND  [Country].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_31_EN [SizeUoM] ON stgl.[SizeUoM] = [SizeUoM].Code AND  [SizeUoM].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_31_EN [WeightUoM] ON stgl.[WeightUoM] = [WeightUoM].Code AND  [WeightUoM].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_32_EN [InHouseManufacture] ON stgl.[InHouseManufacture] = [InHouseManufacture].Code AND  [InHouseManufacture].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_32_EN [FinishedGoodIndicator] ON stgl.[FinishedGoodIndicator] = [FinishedGoodIndicator].Code AND  [FinishedGoodIndicator].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_32_EN [DiscontinuedItemInd] ON stgl.[DiscontinuedItemInd] = [DiscontinuedItemInd].Code AND  [DiscontinuedItemInd].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_27_EN [ProductLine] ON stgl.[ProductLine] = [ProductLine].Code AND  [ProductLine].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_23_EN [DealerCostCurrencyCode] ON stgl.[DealerCostCurrencyCode] = [DealerCostCurrencyCode].Code AND  [DealerCostCurrencyCode].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_23_EN [MSRPCurrencyCode] ON stgl.[MSRPCurrencyCode] = [MSRPCurrencyCode].Code AND  [MSRPCurrencyCode].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_3_29_EN [Size] ON stgl.[Size] = [Size].Code AND  [Size].Version_ID = @Version_ID  
...
,[uda_24_714] --[Size]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_24_690]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_691]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_692]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_693]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_694]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_695]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_696]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_697]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_698]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_699]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_700]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_701]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_702]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_703]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_704]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_705]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_706]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_707]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_708]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_709]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_710]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_711]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_712]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_713]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_714])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
SELECT DISTINCT ID AS Hierarchy_ID FROM mdm.tblHierarchy WHERE Entity_ID = 24 AND IsMandatory = 1  
...
WHILE EXISTS(SELECT 1 FROM @MandatoryHierarchy) BEGIN  
    ...
    BEGIN  
        ...
        SELECT     
            ...
            GETUTCDATE(),     
            @User_ID    
        FROM mdm.[tbl_3_24_EN] en INNER JOIN [stg].[Product_Leaf] stgl   
...
WHILE @TrackGroupCount <= @TrackGroupMax BEGIN  
    ...
    FROM mdm.[tbl_3_24_EN] en  
        INNER JOIN [stg].[Product_Leaf] stgl  
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[Product_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_3_24_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_24_690] = CASE WHEN stgl.[ProductSubCategory] = @NULLText THEN NULL   
            ELSE ISNULL([ProductSubCategory].ID, en.[uda_24_690]) END  
            ,[uda_24_691] = CASE WHEN stgl.[Color] = @NULLText THEN NULL   
            ELSE ISNULL([Color].ID, en.[uda_24_691]) END  
            ,[uda_24_692] = CASE WHEN stgl.[Class] = @NULLText THEN NULL   
            ELSE ISNULL([Class].ID, en.[uda_24_692]) END  
            ,[uda_24_693] = CASE WHEN stgl.[Style] = @NULLText THEN NULL   
            ELSE ISNULL([Style].ID, en.[uda_24_693]) END  
            ,[uda_24_694] = CASE WHEN stgl.[Country] = @NULLText THEN NULL   
            ELSE ISNULL([Country].ID, en.[uda_24_694]) END  
            ,[uda_24_705] = CASE WHEN stgl.[SizeUoM] = @NULLText THEN NULL   
            ELSE ISNULL([SizeUoM].ID, en.[uda_24_705]) END  
            ,[uda_24_706] = CASE WHEN stgl.[WeightUoM] = @NULLText THEN NULL   
            ELSE ISNULL([WeightUoM].ID, en.[uda_24_706]) END  
            ,[uda_24_707] = CASE WHEN stgl.[InHouseManufacture] = @NULLText THEN NULL   
            ELSE ISNULL([InHouseManufacture].ID, en.[uda_24_707]) END  
            ,[uda_24_708] = CASE WHEN stgl.[FinishedGoodIndicator] = @NULLText THEN NULL   
            ELSE ISNULL([FinishedGoodIndicator].ID, en.[uda_24_708]) END  
            ,[uda_24_709] = CASE WHEN stgl.[DiscontinuedItemInd] = @NULLText THEN NULL   
            ELSE ISNULL([DiscontinuedItemInd].ID, en.[uda_24_709]) END  
            ,[uda_24_711] = CASE WHEN stgl.[ProductLine] = @NULLText THEN NULL   
            ELSE ISNULL([ProductLine].ID, en.[uda_24_711]) END  
            ,[uda_24_712] = CASE WHEN stgl.[DealerCostCurrencyCode] = @NULLText THEN NULL   
            ELSE ISNULL([DealerCostCurrencyCode].ID, en.[uda_24_712]) END  
            ,[uda_24_713] = CASE WHEN stgl.[MSRPCurrencyCode] = @NULLText THEN NULL   
            ELSE ISNULL([MSRPCurrencyCode].ID, en.[uda_24_713]) END  
            ,[uda_24_714] = CASE WHEN stgl.[Size] = @NULLText THEN NULL   
            ELSE ISNULL([Size].ID, en.[uda_24_714]) END  
              
        ,[uda_24_695] = CASE WHEN stgl.[StandardCost] = @NULLNumber THEN NULL  
                                               WHEN stgl.[StandardCost] IS NULL THEN en.[uda_24_695]  
                                               ELSE stgl.[StandardCost]  
                                          END ,[uda_24_696] = CASE WHEN stgl.[SafetyStockLevel] = @NULLNumber THEN NULL  
                                               WHEN stgl.[SafetyStockLevel] IS NULL THEN en.[uda_24_696]  
                                               ELSE stgl.[SafetyStockLevel]  
                                          END ,[uda_24_697] = CASE WHEN stgl.[ReorderPoint] = @NULLNumber THEN NULL  
                                               WHEN stgl.[ReorderPoint] IS NULL THEN en.[uda_24_697]  
                                               ELSE stgl.[ReorderPoint]  
                                          END ,[uda_24_698] = CASE WHEN stgl.[MSRP] = @NULLNumber THEN NULL  
                                               WHEN stgl.[MSRP] IS NULL THEN en.[uda_24_698]  
                                               ELSE stgl.[MSRP]  
                                          END ,[uda_24_699] = CASE WHEN stgl.[Weight] = @NULLNumber THEN NULL  
                                               WHEN stgl.[Weight] IS NULL THEN en.[uda_24_699]  
                                               ELSE stgl.[Weight]  
                                          END ,[uda_24_700] = CASE WHEN stgl.[DaysToManufacture] = @NULLNumber THEN NULL  
                                               WHEN stgl.[DaysToManufacture] IS NULL THEN en.[uda_24_700]  
                                               ELSE stgl.[DaysToManufacture]  
                                          END ,[uda_24_701] = CASE WHEN stgl.[DealerCost] = @NULLNumber THEN NULL  
                                               WHEN stgl.[DealerCost] IS NULL THEN en.[uda_24_701]  
                                               ELSE stgl.[DealerCost]  
                                          END ,[uda_24_702] = CASE WHEN stgl.[DocumentationURL] = @NULLText THEN NULL  
                                               WHEN stgl.[DocumentationURL] IS NULL THEN en.[uda_24_702]  
                                               ELSE stgl.[DocumentationURL]  
                                          END ,[uda_24_703] = CASE WHEN stgl.[SellStartDate] = @NULLDateTime THEN NULL  
                                               WHEN stgl.[SellStartDate] IS NULL THEN en.[uda_24_703]  
                                               ELSE stgl.[SellStartDate]  
                                          END ,[uda_24_704] = CASE WHEN stgl.[SellEndDate] = @NULLDateTime THEN NULL  
                                               WHEN stgl.[SellEndDate] IS NULL THEN en.[uda_24_704]  
                                               ELSE stgl.[SellEndDate]  
                                          END ,[uda_24_710] = CASE WHEN stgl.[DiscontiuedDate] = @NULLDateTime THEN NULL  
                                               WHEN stgl.[DiscontiuedDate] IS NULL THEN en.[uda_24_710]  
                                               ELSE stgl.[DiscontiuedDate]  
                                          END   
    FROM mdm.[tbl_3_24_EN] en INNER JOIN [stg].[Product_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_24_695] = CASE WHEN stgl.[StandardCost] = @NULLNumber THEN NULL  
                                          ...
                                          END ,[uda_24_710] = CASE WHEN stgl.[DiscontiuedDate] = @NULLDateTime THEN NULL  
                                               ...
                                               ELSE stgl.[DiscontiuedDate]  
                                          END   
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_24_690]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_691]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_692]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_693]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_694]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_695]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_696]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_697]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_698]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_699]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_700]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_701]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_702]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_703]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_704]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_705]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_706]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_707]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_708]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_709]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_710]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_711]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_712]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_713]) , CONVERT(NVARCHAR(MAX), deleted.[uda_24_714]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_690]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_691]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_692]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_693]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_694]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_695]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_696]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_697]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_698]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_699]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_700]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_701]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_702]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_703]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_704]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_705]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_706]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_707]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_708]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_709]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_710]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_711]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_712]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_713]) , CONVERT(NVARCHAR(MAX), inserted.[uda_24_714])   
        INTO #TRANLOG  
    FROM mdm.[tbl_3_24_EN] en INNER JOIN [stg].[Product_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
BEGIN         
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,LastChgVersionID = @Version_ID  
        ,[uda_24_690] = CASE WHEN stgl.[ProductSubCategory] = @NULLText THEN NULL   
            ELSE [ProductSubCategory].ID END  
            ,[uda_24_691] = CASE WHEN stgl.[Color] = @NULLText THEN NULL   
            ELSE [Color].ID END  
            ,[uda_24_692] = CASE WHEN stgl.[Class] = @NULLText THEN NULL   
            ELSE [Class].ID END  
            ...
            ELSE [SizeUoM].ID END  
            ,[uda_24_706] = CASE WHEN stgl.[WeightUoM] = @NULLText THEN NULL   
            ELSE [WeightUoM].ID END  
            ,[uda_24_707] = CASE WHEN stgl.[InHouseManufacture] = @NULLText THEN NULL   
            ELSE [InHouseManufacture].ID END  
            ,[uda_24_708] = CASE WHEN stgl.[FinishedGoodIndicator] = @NULLText THEN NULL   
            ELSE [FinishedGoodIndicator].ID END  
            ,[uda_24_709] = CASE WHEN stgl.[DiscontinuedItemInd] = @NULLText THEN NULL   
            ELSE [DiscontinuedItemInd].ID END  
            ,[uda_24_711] = CASE WHEN stgl.[ProductLine] = @NULLText THEN NULL   
            ELSE [ProductLine].ID END  
            ,[uda_24_712] = CASE WHEN stgl.[DealerCostCurrencyCode] = @NULLText THEN NULL   
            ELSE [DealerCostCurrencyCode].ID END  
            ,[uda_24_713] = CASE WHEN stgl.[MSRPCurrencyCode] = @NULLText THEN NULL   
            ELSE [MSRPCurrencyCode].ID END  
            ,[uda_24_714] = CASE WHEN stgl.[Size] = @NULLText THEN NULL   
            ELSE [Size].ID END  
              
        ,[uda_24_695] = NULLIF(stgl.[StandardCost],@NULLNumber),[uda_24_696] = NULLIF(stgl.[SafetyStockLevel],@NULLNumber),[uda_24_697] = NULLIF(stgl.[ReorderPoint],@NULLNumber),[uda_24_698] = NULLIF(stgl.[MSRP],@NULLNumber),[uda_24_699] = NULLIF(stgl.[Weight],@NULLNumber),[uda_24_700] = NULLIF(stgl.[DaysToManufacture],@NULLNumber),[uda_24_701] = NULLIF(stgl.[DealerCost],@NULLNumber),[uda_24_702] = NULLIF(stgl.[DocumentationURL],@NULLText),[uda_24_703] = NULLIF(stgl.[SellStartDate],@NULLDateTime),[uda_24_704] = NULLIF(stgl.[SellEndDate],@NULLDateTime),[uda_24_710] = NULLIF(stgl.[DiscontiuedDate],@NULLDateTime)  
    FROM mdm.[tbl_3_24_EN] en INNER JOIN [stg].[Product_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[Product_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[Product_Leaf]   
...
BEGIN  
    ...
    UPDATE mdm.[tbl_3_24_HR]  
    SET Status_ID = @MemberStatusInactive  
    FROM mdm.[tbl_3_24_HR] hr  
    INNER JOIN mdm.[tbl_3_24_EN] en   
    ON hr.Child_EN_ID = en.ID AND hr.Version_ID = @Version_ID  
    INNER JOIN [stg].[Product_Leaf] stgl  
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
    AND stgl.ImportStatus_ID = @StatusProcessing AND stgl.Batch_ID = @Batch_ID  
