...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_Area_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[Area_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[Area_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[Area_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[Area_Leaf] stgl  
    INNER JOIN mdm.[tbl_2_7_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[BigArea]', inserted.[BigArea], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Area_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_8_EN dm   
    ON stgl.[BigArea] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[BigArea],  @NULLText), N'')) > 0   
...
FROM [stg].[Area_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_2_7_EN] en   
...
FROM [stg].[Area_Leaf] sc  
    INNER JOIN mdm.[tbl_2_7_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Area_Leaf] sc  
    INNER JOIN mdm.[tbl_2_7_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[Area_Leaf]  
...
(  
            ...
            ,uda_7_315 NVARCHAR(MAX) NULL   
            ,New_uda_7_315 NVARCHAR(MAX) NULL   
);  
...
,[uda_7_315] --[BigArea]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[BigArea] = @NULLText THEN NULL ELSE [BigArea].ID END  
    FROM [stg].[Area_Leaf] stgl   
LEFT JOIN mdm.tbl_2_8_EN [BigArea] ON stgl.[BigArea] = [BigArea].Code AND  [BigArea].Version_ID = @Version_ID  
...
,[uda_7_315] --[BigArea]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_7_315])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_2_7_EN] en INNER JOIN [stg].[Area_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[Area_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_2_7_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_7_315] = CASE WHEN stgl.[BigArea] = @NULLText THEN NULL   
            ELSE ISNULL([BigArea].ID, en.[uda_7_315]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_7_315]) , CONVERT(NVARCHAR(MAX), inserted.[uda_7_315])   
        INTO #TRANLOG  
    FROM mdm.[tbl_2_7_EN] en INNER JOIN [stg].[Area_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[Area_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[Area_Leaf]   
