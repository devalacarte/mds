...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_Group_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[Group_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[Group_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[Group_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[Group_Leaf] stgl  
    INNER JOIN mdm.[tbl_4_37_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[SubClass]', inserted.[SubClass], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Group_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_4_41_EN dm   
    ON stgl.[SubClass] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[SubClass],  @NULLText), N'')) > 0   
...
FROM [stg].[Group_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_4_37_EN] en   
...
FROM [stg].[Group_Leaf] sc  
    INNER JOIN mdm.[tbl_4_37_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Group_Leaf] sc  
    INNER JOIN mdm.[tbl_4_37_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[Group_Leaf]  
...
(  
            ...
            ,uda_37_940 NVARCHAR(MAX) NULL   
            ,New_uda_37_940 NVARCHAR(MAX) NULL   
);  
...
,[uda_37_940] --[SubClass]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[SubClass] = @NULLText THEN NULL ELSE [SubClass].ID END  
    FROM [stg].[Group_Leaf] stgl   
LEFT JOIN mdm.tbl_4_41_EN [SubClass] ON stgl.[SubClass] = [SubClass].Code AND  [SubClass].Version_ID = @Version_ID  
...
,[uda_37_940] --[SubClass]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_37_940])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_4_37_EN] en INNER JOIN [stg].[Group_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[Group_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_4_37_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_37_940] = CASE WHEN stgl.[SubClass] = @NULLText THEN NULL   
            ELSE ISNULL([SubClass].ID, en.[uda_37_940]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_37_940]) , CONVERT(NVARCHAR(MAX), inserted.[uda_37_940])   
        INTO #TRANLOG  
    FROM mdm.[tbl_4_37_EN] en INNER JOIN [stg].[Group_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[Group_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[Group_Leaf]   
