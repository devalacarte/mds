...
AS BEGIN    
    ...
    EXEC udpEntityStagingDeleteStoredProcedures @Entity_ID, 1    
                      
    SET @SQL = @TruncationGuard +  
N'CREATE PROCEDURE [stg].[udp_' + @StagingBase + N'_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].' + @StagingLeafTable + N' stgl  
...
'    
    ...
    SELECT    
        ...
        SortOrder      
    FROM           
        mdm.udfEntityAttributesGetList(@Entity_ID, 1)    
...
'  
            ...
            SET @SQLDBAJoin += N'  
LEFT JOIN mdm.' + @CurrentDomainTable + N' ' + @CurrentViewColumn + N' ON stgl.' + @CurrentViewColumn + N' = ' + @CurrentViewColumn + N'.Code AND ' + N' ' + @CurrentViewColumn + N'.Version_ID = @Version_ID  
'    
...
WHERE Code in (SELECT Code FROM [stg].' + @StagingLeafTable + N' WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].' + @StagingLeafTable + N' stgl  
    LEFT OUTER JOIN mdm.' + @EntityTable + N' en   
...
IF LEN(COALESCE(@HierarchyParentTable, N'')) > 0 BEGIN       
    SET @SQL += @TruncationGuard + N'  
--Error 300003 Binary Location 2^9: The Member code already exists  
...
BEGIN  
    ...
    (  
        ...
        LastChgDTM,  
        LastChgUserID,  
        LastChgVersionID' + COALESCE(@SQLNonDBAColumns, N'') + COALESCE(@SQLDBAColumns, N'') + N'  
    )  
    SELECT  
        @Version_ID,  
        1,  
        ...
        @Version_ID' +   
            ...
            END    
            + N'  
    FROM [stg].' + @StagingLeafTable + N' stgl ' + COALESCE(@SQLDBAJoin, N'') + N'  
...
SELECT DISTINCT ID AS Hierarchy_ID FROM mdm.tblHierarchy WHERE Entity_ID = ' + CONVERT(NVARCHAR(30), @Entity_ID) + N' AND IsMandatory = 1  
...
WHILE EXISTS(SELECT 1 FROM @MandatoryHierarchy) BEGIN  
    ...
    BEGIN  
        ...
        SELECT     
            ...
            GETUTCDATE(),     
            @User_ID    
        FROM mdm.' + @EntityTable + N' en INNER JOIN [stg].' + @StagingLeafTable + N' stgl   
...
UPDATE [stg].' + @StagingLeafTable + N'  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.' + @EntityTable + N' WHERE Version_ID = @Version_ID)  
...
IF Exists(SELECT Code FROM [stg].' + @StagingLeafTable + N'   
