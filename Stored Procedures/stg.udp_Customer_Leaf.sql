...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_Customer_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[Customer_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[Customer_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[Customer_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[Customer_Leaf] stgl  
    INNER JOIN mdm.[tbl_2_12_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[StateProvince]', inserted.[StateProvince], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_18_EN dm   
    ON stgl.[StateProvince] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[StateProvince],  @NULLText), N'')) > 0   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    ...
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_9_EN dm   
...
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[CustomerType]', inserted.[CustomerType], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_13_EN dm   
    ON stgl.[CustomerType] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[CustomerType],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Salutation]', inserted.[Salutation], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_17_EN dm   
    ON stgl.[Salutation] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[Salutation],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[BillingCurrency]', inserted.[BillingCurrency], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_11_EN dm   
    ON stgl.[BillingCurrency] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[BillingCurrency],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[PaymentTerms]', inserted.[PaymentTerms], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_10_EN dm   
    ON stgl.[PaymentTerms] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[PaymentTerms],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[SalesDistrict]', inserted.[SalesDistrict], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_15_EN dm   
    ON stgl.[SalesDistrict] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[SalesDistrict],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[AddressType]', inserted.[AddressType], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_6_EN dm   
    ON stgl.[AddressType] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[AddressType],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
...
FROM [stg].[Customer_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_2_12_EN] en   
...
FROM [stg].[Customer_Leaf] sc  
    INNER JOIN mdm.[tbl_2_12_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Customer_Leaf] sc  
    INNER JOIN mdm.[tbl_2_12_EN] AS tSource ON sc.NewCode = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Customer_Leaf] sc  
    INNER JOIN mdm.[tbl_2_12_HP] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Customer_Leaf] sc  
    INNER JOIN mdm.[tbl_2_12_HP] AS tSource ON sc.NewCode = tSource.Code  
WHERE tSource.Version_ID = @Version_ID AND sc.ImportType IN (@IT_MergeOptimistic, @IT_MergeOverwrite)  
    AND sc.ImportStatus_ID = @StatusDefault AND sc.Batch_ID = @Batch_ID;  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Customer_Leaf] sc  
    INNER JOIN mdm.[tbl_2_12_CN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Customer_Leaf] sc  
    INNER JOIN mdm.[tbl_2_12_CN] AS tSource ON sc.NewCode = tSource.Code  
WHERE tSource.Version_ID = @Version_ID AND sc.ImportType IN (@IT_MergeOptimistic, @IT_MergeOverwrite)  
    AND sc.ImportStatus_ID = @StatusDefault AND sc.Batch_ID = @Batch_ID;  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[Customer_Leaf]  
...
(  
            ...
            ,uda_12_389 NVARCHAR(MAX) NULL   
            ,uda_12_390 NVARCHAR(MAX) NULL   
            ,uda_12_391 NVARCHAR(MAX) NULL   
            ,uda_12_392 NVARCHAR(MAX) NULL   
            ,uda_12_393 NVARCHAR(MAX) NULL   
            ,uda_12_394 NVARCHAR(MAX) NULL   
            ,uda_12_395 NVARCHAR(MAX) NULL   
            ,uda_12_396 NVARCHAR(MAX) NULL   
            ,uda_12_397 NVARCHAR(MAX) NULL   
            ,uda_12_398 NVARCHAR(MAX) NULL   
            ,uda_12_399 NVARCHAR(MAX) NULL   
            ,uda_12_400 NVARCHAR(MAX) NULL   
            ,uda_12_401 NVARCHAR(MAX) NULL   
            ,uda_12_402 NVARCHAR(MAX) NULL   
            ,uda_12_403 NVARCHAR(MAX) NULL   
            ,uda_12_404 NVARCHAR(MAX) NULL   
            ,New_uda_12_389 NVARCHAR(MAX) NULL   
            ,New_uda_12_390 NVARCHAR(MAX) NULL   
            ,New_uda_12_391 NVARCHAR(MAX) NULL   
            ,New_uda_12_392 NVARCHAR(MAX) NULL   
            ,New_uda_12_393 NVARCHAR(MAX) NULL   
            ,New_uda_12_394 NVARCHAR(MAX) NULL   
            ,New_uda_12_395 NVARCHAR(MAX) NULL   
            ,New_uda_12_396 NVARCHAR(MAX) NULL   
            ,New_uda_12_397 NVARCHAR(MAX) NULL   
            ,New_uda_12_398 NVARCHAR(MAX) NULL   
            ,New_uda_12_399 NVARCHAR(MAX) NULL   
            ,New_uda_12_400 NVARCHAR(MAX) NULL   
            ,New_uda_12_401 NVARCHAR(MAX) NULL   
            ,New_uda_12_402 NVARCHAR(MAX) NULL   
            ,New_uda_12_403 NVARCHAR(MAX) NULL   
            ,New_uda_12_404 NVARCHAR(MAX) NULL   
);  
...
BEGIN  
    ...
    (  
        ...
        LastChgUserID,  
        LastChgVersionID  
,[uda_12_389] --[AddressLine1]  
,[uda_12_390] --[AddressLine2]  
,[uda_12_391] --[AddressLine3]  
,[uda_12_394] --[PostalCode]  
,[uda_12_395] --[Telephone]  
,[uda_12_396] --[Email]  
,[uda_12_397] --[Website]  
,[uda_12_404] --[City]     
,[uda_12_392] --[StateProvince]     
,[uda_12_393] --[Country]     
,[uda_12_398] --[CustomerType]     
,[uda_12_399] --[Salutation]     
,[uda_12_400] --[BillingCurrency]     
,[uda_12_401] --[PaymentTerms]     
,[uda_12_402] --[SalesDistrict]     
,[uda_12_403] --[AddressType]  
    )  
    SELECT  
        @Version_ID,  
        1,  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,NULLIF(stgl.[AddressLine1],@NULLText),  
NULLIF(stgl.[AddressLine2],@NULLText),  
NULLIF(stgl.[AddressLine3],@NULLText),  
NULLIF(stgl.[PostalCode],@NULLText),  
NULLIF(stgl.[Telephone],@NULLText),  
NULLIF(stgl.[Email],@NULLText),  
NULLIF(stgl.[Website],@NULLText),  
NULLIF(stgl.[City],@NULLText),CASE WHEN stgl.[StateProvince] = @NULLText THEN NULL ELSE [StateProvince].ID END,  
CASE WHEN stgl.[Country] = @NULLText THEN NULL ELSE [Country].ID END,  
CASE WHEN stgl.[CustomerType] = @NULLText THEN NULL ELSE [CustomerType].ID END,  
CASE WHEN stgl.[Salutation] = @NULLText THEN NULL ELSE [Salutation].ID END,  
CASE WHEN stgl.[BillingCurrency] = @NULLText THEN NULL ELSE [BillingCurrency].ID END,  
CASE WHEN stgl.[PaymentTerms] = @NULLText THEN NULL ELSE [PaymentTerms].ID END,  
CASE WHEN stgl.[SalesDistrict] = @NULLText THEN NULL ELSE [SalesDistrict].ID END,  
CASE WHEN stgl.[AddressType] = @NULLText THEN NULL ELSE [AddressType].ID END  
    FROM [stg].[Customer_Leaf] stgl   
LEFT JOIN mdm.tbl_2_18_EN [StateProvince] ON stgl.[StateProvince] = [StateProvince].Code AND  [StateProvince].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_2_9_EN [Country] ON stgl.[Country] = [Country].Code AND  [Country].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_2_13_EN [CustomerType] ON stgl.[CustomerType] = [CustomerType].Code AND  [CustomerType].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_2_17_EN [Salutation] ON stgl.[Salutation] = [Salutation].Code AND  [Salutation].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_2_11_EN [BillingCurrency] ON stgl.[BillingCurrency] = [BillingCurrency].Code AND  [BillingCurrency].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_2_10_EN [PaymentTerms] ON stgl.[PaymentTerms] = [PaymentTerms].Code AND  [PaymentTerms].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_2_15_EN [SalesDistrict] ON stgl.[SalesDistrict] = [SalesDistrict].Code AND  [SalesDistrict].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_2_6_EN [AddressType] ON stgl.[AddressType] = [AddressType].Code AND  [AddressType].Version_ID = @Version_ID  
...
,[uda_12_403] --[AddressType]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_12_389]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_390]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_391]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_392]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_393]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_394]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_395]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_396]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_397]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_398]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_399]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_400]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_401]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_402]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_403]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_404])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
SELECT DISTINCT ID AS Hierarchy_ID FROM mdm.tblHierarchy WHERE Entity_ID = 12 AND IsMandatory = 1  
...
WHILE EXISTS(SELECT 1 FROM @MandatoryHierarchy) BEGIN  
    ...
    BEGIN  
        ...
        SELECT     
            ...
            GETUTCDATE(),     
            @User_ID    
        FROM mdm.[tbl_2_12_EN] en INNER JOIN [stg].[Customer_Leaf] stgl   
...
WHILE @TrackGroupCount <= @TrackGroupMax BEGIN  
    ...
    FROM mdm.[tbl_2_12_EN] en  
        INNER JOIN [stg].[Customer_Leaf] stgl  
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[Customer_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_2_12_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_12_392] = CASE WHEN stgl.[StateProvince] = @NULLText THEN NULL   
            ELSE ISNULL([StateProvince].ID, en.[uda_12_392]) END  
            ,[uda_12_393] = CASE WHEN stgl.[Country] = @NULLText THEN NULL   
            ELSE ISNULL([Country].ID, en.[uda_12_393]) END  
            ,[uda_12_398] = CASE WHEN stgl.[CustomerType] = @NULLText THEN NULL   
            ELSE ISNULL([CustomerType].ID, en.[uda_12_398]) END  
            ,[uda_12_399] = CASE WHEN stgl.[Salutation] = @NULLText THEN NULL   
            ELSE ISNULL([Salutation].ID, en.[uda_12_399]) END  
            ,[uda_12_400] = CASE WHEN stgl.[BillingCurrency] = @NULLText THEN NULL   
            ELSE ISNULL([BillingCurrency].ID, en.[uda_12_400]) END  
            ,[uda_12_401] = CASE WHEN stgl.[PaymentTerms] = @NULLText THEN NULL   
            ELSE ISNULL([PaymentTerms].ID, en.[uda_12_401]) END  
            ,[uda_12_402] = CASE WHEN stgl.[SalesDistrict] = @NULLText THEN NULL   
            ELSE ISNULL([SalesDistrict].ID, en.[uda_12_402]) END  
            ,[uda_12_403] = CASE WHEN stgl.[AddressType] = @NULLText THEN NULL   
            ELSE ISNULL([AddressType].ID, en.[uda_12_403]) END  
              
        ,[uda_12_389] = CASE WHEN stgl.[AddressLine1] = @NULLText THEN NULL  
                                               WHEN stgl.[AddressLine1] IS NULL THEN en.[uda_12_389]  
                                               ELSE stgl.[AddressLine1]  
                                          END ,[uda_12_390] = CASE WHEN stgl.[AddressLine2] = @NULLText THEN NULL  
                                               WHEN stgl.[AddressLine2] IS NULL THEN en.[uda_12_390]  
                                               ELSE stgl.[AddressLine2]  
                                          END ,[uda_12_391] = CASE WHEN stgl.[AddressLine3] = @NULLText THEN NULL  
                                               WHEN stgl.[AddressLine3] IS NULL THEN en.[uda_12_391]  
                                               ELSE stgl.[AddressLine3]  
                                          END ,[uda_12_394] = CASE WHEN stgl.[PostalCode] = @NULLText THEN NULL  
                                               WHEN stgl.[PostalCode] IS NULL THEN en.[uda_12_394]  
                                               ELSE stgl.[PostalCode]  
                                          END ,[uda_12_395] = CASE WHEN stgl.[Telephone] = @NULLText THEN NULL  
                                               WHEN stgl.[Telephone] IS NULL THEN en.[uda_12_395]  
                                               ELSE stgl.[Telephone]  
                                          END ,[uda_12_396] = CASE WHEN stgl.[Email] = @NULLText THEN NULL  
                                               WHEN stgl.[Email] IS NULL THEN en.[uda_12_396]  
                                               ELSE stgl.[Email]  
                                          END ,[uda_12_397] = CASE WHEN stgl.[Website] = @NULLText THEN NULL  
                                               WHEN stgl.[Website] IS NULL THEN en.[uda_12_397]  
                                               ELSE stgl.[Website]  
                                          END ,[uda_12_404] = CASE WHEN stgl.[City] = @NULLText THEN NULL  
                                               WHEN stgl.[City] IS NULL THEN en.[uda_12_404]  
                                               ELSE stgl.[City]  
                                          END   
    FROM mdm.[tbl_2_12_EN] en INNER JOIN [stg].[Customer_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_12_389] = CASE WHEN stgl.[AddressLine1] = @NULLText THEN NULL  
                                          ...
                                          END ,[uda_12_404] = CASE WHEN stgl.[City] = @NULLText THEN NULL  
                                               ...
                                               ELSE stgl.[City]  
                                          END   
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_12_389]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_390]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_391]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_392]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_393]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_394]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_395]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_396]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_397]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_398]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_399]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_400]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_401]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_402]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_403]) , CONVERT(NVARCHAR(MAX), deleted.[uda_12_404]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_389]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_390]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_391]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_392]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_393]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_394]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_395]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_396]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_397]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_398]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_399]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_400]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_401]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_402]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_403]) , CONVERT(NVARCHAR(MAX), inserted.[uda_12_404])   
        INTO #TRANLOG  
    FROM mdm.[tbl_2_12_EN] en INNER JOIN [stg].[Customer_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
BEGIN         
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,LastChgVersionID = @Version_ID  
        ,[uda_12_392] = CASE WHEN stgl.[StateProvince] = @NULLText THEN NULL   
            ELSE [StateProvince].ID END  
            ,[uda_12_393] = CASE WHEN stgl.[Country] = @NULLText THEN NULL   
            ELSE [Country].ID END  
            ,[uda_12_398] = CASE WHEN stgl.[CustomerType] = @NULLText THEN NULL   
            ELSE [CustomerType].ID END  
            ,[uda_12_399] = CASE WHEN stgl.[Salutation] = @NULLText THEN NULL   
            ELSE [Salutation].ID END  
            ,[uda_12_400] = CASE WHEN stgl.[BillingCurrency] = @NULLText THEN NULL   
            ELSE [BillingCurrency].ID END  
            ,[uda_12_401] = CASE WHEN stgl.[PaymentTerms] = @NULLText THEN NULL   
            ELSE [PaymentTerms].ID END  
            ,[uda_12_402] = CASE WHEN stgl.[SalesDistrict] = @NULLText THEN NULL   
            ELSE [SalesDistrict].ID END  
            ,[uda_12_403] = CASE WHEN stgl.[AddressType] = @NULLText THEN NULL   
            ELSE [AddressType].ID END  
              
        ,[uda_12_389] = NULLIF(stgl.[AddressLine1],@NULLText),[uda_12_390] = NULLIF(stgl.[AddressLine2],@NULLText),[uda_12_391] = NULLIF(stgl.[AddressLine3],@NULLText),[uda_12_394] = NULLIF(stgl.[PostalCode],@NULLText),[uda_12_395] = NULLIF(stgl.[Telephone],@NULLText),[uda_12_396] = NULLIF(stgl.[Email],@NULLText),[uda_12_397] = NULLIF(stgl.[Website],@NULLText),[uda_12_404] = NULLIF(stgl.[City],@NULLText)  
    FROM mdm.[tbl_2_12_EN] en INNER JOIN [stg].[Customer_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[Customer_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[Customer_Leaf]   
...
BEGIN  
    ...
    UPDATE mdm.[tbl_2_12_HR]  
    SET Status_ID = @MemberStatusInactive  
    FROM mdm.[tbl_2_12_HR] hr  
    INNER JOIN mdm.[tbl_2_12_EN] en   
