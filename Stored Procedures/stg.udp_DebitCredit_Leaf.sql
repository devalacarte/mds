...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_DebitCredit_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[DebitCredit_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[DebitCredit_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[DebitCredit_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[DebitCredit_Leaf] stgl  
    INNER JOIN mdm.[tbl_4_36_EN] en   
...
FROM [stg].[DebitCredit_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_4_36_EN] en   
...
FROM [stg].[DebitCredit_Leaf] sc  
    INNER JOIN mdm.[tbl_4_36_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[DebitCredit_Leaf] sc  
    INNER JOIN mdm.[tbl_4_36_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[DebitCredit_Leaf]  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_4_36_EN] en INNER JOIN [stg].[DebitCredit_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[DebitCredit_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_4_36_EN] WHERE Version_ID = @Version_ID)  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[DebitCredit_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[DebitCredit_Leaf]   
