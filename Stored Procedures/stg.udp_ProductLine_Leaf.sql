...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_ProductLine_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[ProductLine_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[ProductLine_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[ProductLine_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[ProductLine_Leaf] stgl  
    INNER JOIN mdm.[tbl_3_27_EN] en   
...
FROM [stg].[ProductLine_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_3_27_EN] en   
...
FROM [stg].[ProductLine_Leaf] sc  
    INNER JOIN mdm.[tbl_3_27_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[ProductLine_Leaf] sc  
    INNER JOIN mdm.[tbl_3_27_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[ProductLine_Leaf]  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_3_27_EN] en INNER JOIN [stg].[ProductLine_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[ProductLine_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_3_27_EN] WHERE Version_ID = @Version_ID)  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[ProductLine_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[ProductLine_Leaf]   
