...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_LineItemDetail_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[LineItemDetail_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[LineItemDetail_Leaf] stgl  
...
-- Create a new batch ID.  
    ...
    IF @GetNewBatch_ID = @BatchIDNotFound BEGIN  
        UPDATE [stg].[LineItemDetail_Leaf]  
...
WHERE Code in (SELECT Code FROM [stg].[LineItemDetail_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[LineItemDetail_Leaf] stgl  
    INNER JOIN mdm.[tbl_4_39_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[LineItem]', inserted.[LineItem], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[LineItemDetail_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_4_38_EN dm   
    ON stgl.[LineItem] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[LineItem],  @NULLText), N'')) > 0   
...
FROM [stg].[LineItemDetail_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_4_39_EN] en   
...
FROM [stg].[LineItemDetail_Leaf] sc  
    INNER JOIN mdm.[tbl_4_39_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[LineItemDetail_Leaf] sc  
    INNER JOIN mdm.[tbl_4_39_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[LineItemDetail_Leaf]  
...
(  
            ...
            ,uda_39_942 NVARCHAR(MAX) NULL   
            ,New_uda_39_942 NVARCHAR(MAX) NULL   
);  
...
,[uda_39_942] --[LineItem]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[LineItem] = @NULLText THEN NULL ELSE [LineItem].ID END  
    FROM [stg].[LineItemDetail_Leaf] stgl   
LEFT JOIN mdm.tbl_4_38_EN [LineItem] ON stgl.[LineItem] = [LineItem].Code AND  [LineItem].Version_ID = @Version_ID  
...
,[uda_39_942] --[LineItem]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_39_942])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_4_39_EN] en INNER JOIN [stg].[LineItemDetail_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[LineItemDetail_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_4_39_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_39_942] = CASE WHEN stgl.[LineItem] = @NULLText THEN NULL   
            ELSE ISNULL([LineItem].ID, en.[uda_39_942]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_39_942]) , CONVERT(NVARCHAR(MAX), inserted.[uda_39_942])   
        INTO #TRANLOG  
    FROM mdm.[tbl_4_39_EN] en INNER JOIN [stg].[LineItemDetail_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[LineItemDetail_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[LineItemDetail_Leaf]   
