...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_SubClass_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[SubClass_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[SubClass_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[SubClass_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[SubClass_Leaf] stgl  
    INNER JOIN mdm.[tbl_4_41_EN] en   
...
FROM [stg].[SubClass_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_4_35_EN dm   
...
FROM [stg].[SubClass_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_4_41_EN] en   
...
FROM [stg].[SubClass_Leaf] sc  
    INNER JOIN mdm.[tbl_4_41_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[SubClass_Leaf] sc  
    INNER JOIN mdm.[tbl_4_41_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[SubClass_Leaf]  
...
(  
            ...
            ,uda_41_943 NVARCHAR(MAX) NULL   
            ,New_uda_41_943 NVARCHAR(MAX) NULL   
);  
...
,[uda_41_943] --[Class]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[Class] = @NULLText THEN NULL ELSE [Class].ID END  
    FROM [stg].[SubClass_Leaf] stgl   
LEFT JOIN mdm.tbl_4_35_EN [Class] ON stgl.[Class] = [Class].Code AND  [Class].Version_ID = @Version_ID  
...
,[uda_41_943] --[Class]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_41_943])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_4_41_EN] en INNER JOIN [stg].[SubClass_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[SubClass_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_4_41_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_41_943] = CASE WHEN stgl.[Class] = @NULLText THEN NULL   
            ELSE ISNULL([Class].ID, en.[uda_41_943]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_41_943]) , CONVERT(NVARCHAR(MAX), inserted.[uda_41_943])   
        INTO #TRANLOG  
    FROM mdm.[tbl_4_41_EN] en INNER JOIN [stg].[SubClass_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[SubClass_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[SubClass_Leaf]   
