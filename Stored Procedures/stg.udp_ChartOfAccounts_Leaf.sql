...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_ChartOfAccounts_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[ChartOfAccounts_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[ChartOfAccounts_Leaf] stgl  
...
-- Create a new batch ID.  
    ...
    IF @GetNewBatch_ID = @BatchIDNotFound BEGIN  
        UPDATE [stg].[ChartOfAccounts_Leaf]  
...
WHERE Code in (SELECT Code FROM [stg].[ChartOfAccounts_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[ChartOfAccounts_Leaf] stgl  
    INNER JOIN mdm.[tbl_4_34_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[LineItemDetail]', inserted.[LineItemDetail], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[ChartOfAccounts_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_4_39_EN dm   
    ON stgl.[LineItemDetail] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[LineItemDetail],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[AccountType]', inserted.[AccountType], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[ChartOfAccounts_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_4_33_EN dm   
    ON stgl.[AccountType] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[AccountType],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[DebitCreditInd]', inserted.[DebitCreditInd], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[ChartOfAccounts_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_4_36_EN dm   
    ON stgl.[DebitCreditInd] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[DebitCreditInd],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
  
-- Error 210003 The attribute value references a member that does not exist or is inactive. Binary Location 2^2:  
UPDATE stgl  
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Operator]', inserted.[Operator], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[ChartOfAccounts_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_4_40_EN dm   
    ON stgl.[Operator] = dm.Code  
    AND dm.Version_ID = @Version_ID AND dm.Status_ID = @MemberStatusOK  
WHERE stgl.ImportType NOT IN (@IT_Delete, @IT_Purge, @IT_DeleteSetNullToRef, @IT_PurgeSetNullToRef)   
AND LEN(COALESCE(NULLIF(stgl.[Operator],  @NULLText), N'')) > 0   
AND stgl.Batch_ID = @Batch_ID AND stgl.ImportStatus_ID = @StatusDefault AND dm.Code IS NULL  
...
FROM [stg].[ChartOfAccounts_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_4_34_EN] en   
...
FROM [stg].[ChartOfAccounts_Leaf] sc  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[ChartOfAccounts_Leaf]  
...
(  
            ...
            ,uda_34_934 NVARCHAR(MAX) NULL   
            ,uda_34_935 NVARCHAR(MAX) NULL   
            ,uda_34_936 NVARCHAR(MAX) NULL   
            ,uda_34_937 NVARCHAR(MAX) NULL   
            ,New_uda_34_934 NVARCHAR(MAX) NULL   
            ,New_uda_34_935 NVARCHAR(MAX) NULL   
            ,New_uda_34_936 NVARCHAR(MAX) NULL   
            ,New_uda_34_937 NVARCHAR(MAX) NULL   
);  
...
BEGIN  
    ...
    (  
        ...
        LastChgUserID,  
        LastChgVersionID     
,[uda_34_934] --[LineItemDetail]     
,[uda_34_935] --[AccountType]     
,[uda_34_936] --[DebitCreditInd]     
,[uda_34_937] --[Operator]  
    )  
    SELECT  
        @Version_ID,  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[LineItemDetail] = @NULLText THEN NULL ELSE [LineItemDetail].ID END,  
CASE WHEN stgl.[AccountType] = @NULLText THEN NULL ELSE [AccountType].ID END,  
CASE WHEN stgl.[DebitCreditInd] = @NULLText THEN NULL ELSE [DebitCreditInd].ID END,  
CASE WHEN stgl.[Operator] = @NULLText THEN NULL ELSE [Operator].ID END  
    FROM [stg].[ChartOfAccounts_Leaf] stgl   
LEFT JOIN mdm.tbl_4_39_EN [LineItemDetail] ON stgl.[LineItemDetail] = [LineItemDetail].Code AND  [LineItemDetail].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_4_33_EN [AccountType] ON stgl.[AccountType] = [AccountType].Code AND  [AccountType].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_4_36_EN [DebitCreditInd] ON stgl.[DebitCreditInd] = [DebitCreditInd].Code AND  [DebitCreditInd].Version_ID = @Version_ID  
  
LEFT JOIN mdm.tbl_4_40_EN [Operator] ON stgl.[Operator] = [Operator].Code AND  [Operator].Version_ID = @Version_ID  
...
,[uda_34_937] --[Operator]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , NULL , NULL , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_34_934]) , CONVERT(NVARCHAR(MAX), inserted.[uda_34_935]) , CONVERT(NVARCHAR(MAX), inserted.[uda_34_936]) , CONVERT(NVARCHAR(MAX), inserted.[uda_34_937])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
SELECT DISTINCT ID AS Hierarchy_ID FROM mdm.tblHierarchy WHERE Entity_ID = 34 AND IsMandatory = 1  
...
WHILE EXISTS(SELECT 1 FROM @MandatoryHierarchy) BEGIN  
    ...
    BEGIN  
        ...
        SELECT     
            ...
            GETUTCDATE(),     
            @User_ID    
        FROM mdm.[tbl_4_34_EN] en INNER JOIN [stg].[ChartOfAccounts_Leaf] stgl   
...
WHILE @TrackGroupCount <= @TrackGroupMax BEGIN  
    ...
    FROM mdm.[tbl_4_34_EN] en  
        INNER JOIN [stg].[ChartOfAccounts_Leaf] stgl  
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[ChartOfAccounts_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_4_34_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_34_934] = CASE WHEN stgl.[LineItemDetail] = @NULLText THEN NULL   
            ELSE ISNULL([LineItemDetail].ID, en.[uda_34_934]) END  
            ,[uda_34_935] = CASE WHEN stgl.[AccountType] = @NULLText THEN NULL   
            ELSE ISNULL([AccountType].ID, en.[uda_34_935]) END  
            ,[uda_34_936] = CASE WHEN stgl.[DebitCreditInd] = @NULLText THEN NULL   
            ELSE ISNULL([DebitCreditInd].ID, en.[uda_34_936]) END  
            ,[uda_34_937] = CASE WHEN stgl.[Operator] = @NULLText THEN NULL   
            ELSE ISNULL([Operator].ID, en.[uda_34_937]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_34_934]) , CONVERT(NVARCHAR(MAX), deleted.[uda_34_935]) , CONVERT(NVARCHAR(MAX), deleted.[uda_34_936]) , CONVERT(NVARCHAR(MAX), deleted.[uda_34_937]) , CONVERT(NVARCHAR(MAX), inserted.[uda_34_934]) , CONVERT(NVARCHAR(MAX), inserted.[uda_34_935]) , CONVERT(NVARCHAR(MAX), inserted.[uda_34_936]) , CONVERT(NVARCHAR(MAX), inserted.[uda_34_937])   
        INTO #TRANLOG  
    FROM mdm.[tbl_4_34_EN] en INNER JOIN [stg].[ChartOfAccounts_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
BEGIN         
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,LastChgVersionID = @Version_ID  
        ,[uda_34_934] = CASE WHEN stgl.[LineItemDetail] = @NULLText THEN NULL   
            ELSE [LineItemDetail].ID END  
            ,[uda_34_935] = CASE WHEN stgl.[AccountType] = @NULLText THEN NULL   
            ELSE [AccountType].ID END  
            ,[uda_34_936] = CASE WHEN stgl.[DebitCreditInd] = @NULLText THEN NULL   
            ELSE [DebitCreditInd].ID END  
            ,[uda_34_937] = CASE WHEN stgl.[Operator] = @NULLText THEN NULL   
            ELSE [Operator].ID END  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[ChartOfAccounts_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[ChartOfAccounts_Leaf]   
