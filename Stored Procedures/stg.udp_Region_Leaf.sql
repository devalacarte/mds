...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_Region_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[Region_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[Region_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[Region_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[Region_Leaf] stgl  
    INNER JOIN mdm.[tbl_2_14_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Area]', inserted.[Area], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[Region_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_7_EN dm   
    ON stgl.[Area] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[Area],  @NULLText), N'')) > 0   
...
FROM [stg].[Region_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_2_14_EN] en   
...
FROM [stg].[Region_Leaf] sc  
    INNER JOIN mdm.[tbl_2_14_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[Region_Leaf] sc  
    INNER JOIN mdm.[tbl_2_14_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[Region_Leaf]  
...
(  
            ...
            ,uda_14_405 NVARCHAR(MAX) NULL   
            ,New_uda_14_405 NVARCHAR(MAX) NULL   
);  
...
,[uda_14_405] --[Area]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[Area] = @NULLText THEN NULL ELSE [Area].ID END  
    FROM [stg].[Region_Leaf] stgl   
LEFT JOIN mdm.tbl_2_7_EN [Area] ON stgl.[Area] = [Area].Code AND  [Area].Version_ID = @Version_ID  
...
,[uda_14_405] --[Area]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_14_405])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_2_14_EN] en INNER JOIN [stg].[Region_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[Region_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_2_14_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_14_405] = CASE WHEN stgl.[Area] = @NULLText THEN NULL   
            ELSE ISNULL([Area].ID, en.[uda_14_405]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_14_405]) , CONVERT(NVARCHAR(MAX), inserted.[uda_14_405])   
        INTO #TRANLOG  
    FROM mdm.[tbl_2_14_EN] en INNER JOIN [stg].[Region_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[Region_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[Region_Leaf]   
