...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_SalesDistrict_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[SalesDistrict_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[SalesDistrict_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[SalesDistrict_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[SalesDistrict_Leaf] stgl  
    INNER JOIN mdm.[tbl_2_15_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[SalesLocation]', inserted.[SalesLocation], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[SalesDistrict_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_16_EN dm   
    ON stgl.[SalesLocation] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[SalesLocation],  @NULLText), N'')) > 0   
...
FROM [stg].[SalesDistrict_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_2_15_EN] en   
...
FROM [stg].[SalesDistrict_Leaf] sc  
    INNER JOIN mdm.[tbl_2_15_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[SalesDistrict_Leaf] sc  
    INNER JOIN mdm.[tbl_2_15_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[SalesDistrict_Leaf]  
...
(  
            ...
            ,uda_15_406 NVARCHAR(MAX) NULL   
            ,New_uda_15_406 NVARCHAR(MAX) NULL   
);  
...
,[uda_15_406] --[SalesLocation]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[SalesLocation] = @NULLText THEN NULL ELSE [SalesLocation].ID END  
    FROM [stg].[SalesDistrict_Leaf] stgl   
LEFT JOIN mdm.tbl_2_16_EN [SalesLocation] ON stgl.[SalesLocation] = [SalesLocation].Code AND  [SalesLocation].Version_ID = @Version_ID  
...
,[uda_15_406] --[SalesLocation]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_15_406])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_2_15_EN] en INNER JOIN [stg].[SalesDistrict_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[SalesDistrict_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_2_15_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_15_406] = CASE WHEN stgl.[SalesLocation] = @NULLText THEN NULL   
            ELSE ISNULL([SalesLocation].ID, en.[uda_15_406]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_15_406]) , CONVERT(NVARCHAR(MAX), inserted.[uda_15_406])   
        INTO #TRANLOG  
    FROM mdm.[tbl_2_15_EN] en INNER JOIN [stg].[SalesDistrict_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[SalesDistrict_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[SalesDistrict_Leaf]   
