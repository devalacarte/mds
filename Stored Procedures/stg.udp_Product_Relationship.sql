...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_Product_Relationship]    
...
BEGIN    
    ...
    IF LEN(@BatchTag) > 0 BEGIN  
        SELECT @MemberCount = COUNT(ID) FROM [stg].[Product_Relationship]   
    ...
    IF LEN(@BatchTag) > 0 BEGIN  
        IF EXISTS (SELECT stgr.ID FROM [stg].[Product_Relationship] stgr  
    ...
    BEGIN TRY  
        ...
        -- Create a new batch ID.  
            ...
            IF @GetNewBatch_ID = @BatchIDNotFound BEGIN  
                UPDATE [stg].[Product_Relationship]  
        ...
        UPDATE [stg].[Product_Relationship]  
            ...
            WHERE ChildCode IN  
                (SELECT DISTINCT stgr.ChildCode FROM [stg].[Product_Relationship] stgr  
                INNER JOIN mdm.[tbl_3_24_EN] en ON stgr.ChildCode = en.Code WHERE en.Version_ID = @Version_ID AND en.Status_ID = 2)  
        ...
        UPDATE [stg].[Product_Relationship]  
            ...
            WHERE ChildCode NOT IN  
                (SELECT DISTINCT stgr.ChildCode FROM [stg].[Product_Relationship] stgr  
                INNER JOIN mdm.[tbl_3_24_EN] en ON stgr.ChildCode = en.Code WHERE en.Version_ID = @Version_ID)  
        ...
        UPDATE [stg].[Product_Relationship]  
            ...
            WHERE ParentCode <> N'ROOT' AND ParentCode NOT IN  
                (SELECT DISTINCT stgr.ParentCode FROM [stg].[Product_Relationship] stgr  
                 INNER JOIN mdm.[tbl_3_24_HP] hp   
        ...
        UPDATE [stg].[Product_Relationship]  
            ...
            WHERE ParentCode IN  
                (SELECT DISTINCT stgr.ParentCode FROM [stg].[Product_Relationship] stgr  
                INNER JOIN mdm.[tbl_3_24_EN] en ON stgr.ParentCode = en.Code WHERE en.Version_ID = @Version_ID)  
                AND ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID AND RelationshipType = 1;  
        ...
        WHILE EXISTS(SELECT 1 FROM @tblMeta) BEGIN  
            ...
            */  
            ...
            INNER JOIN mdm.[tbl_3_24_EN] AS tSource   
            ...
            FROM #tblStage AS tStage  
            INNER JOIN mdm.[tbl_3_24_HP] AS tSource   
                ON tStage.Member_Code = tSource.Code   
            WHERE tSource.Version_ID = @Version_ID  
            ...
            */  
            ...
            IF @TargetType_ID = 2 BEGIN  
                ...
                FROM #tblStage AS tStage   
                INNER JOIN mdm.[tbl_3_24_HR] AS tRel   
            ...
            FROM #tblStage AS tStage   
            INNER JOIN mdm.[tbl_3_24_HR] AS tSource   
            ...
            */              
            ...
            SELECT @MaxSortOrderRelationship = MAX(tSource.SortOrder)   
                    FROM mdm.[tbl_3_24_HR] AS tSource  
            ...
            */  
            ...
            FROM [stg].[Product_Relationship] AS stgr  
        ...
        */  
        ...
        SELECT @ErrorCount = COUNT(ID) FROM [stg].[Product_Relationship]  
