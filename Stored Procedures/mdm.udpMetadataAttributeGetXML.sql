...
==============================================================================  
*/  
CREATE PROCEDURE [mdm].[udpMetadataAttributeGetXML]   
(  
	@User_ID        INT,  
...
SELECT   
   @ResultOption = mdm.udfMetadataResultOptionGetByName(T.Criteria.value('Attributes[1]', 'nvarchar(50)'))  
...
IF @ResultOption = 0  
	SELECT CONVERT(XML,'<ArrayOfMetadataAttribute/>')  
ELSE IF @ResultOption = 1  
    BEGIN  
        EXEC mdm.udpMetadataAttributeGetIdentifiersXML  
		    @User_ID,   
		    @ModelIDs,   
		    @EntityIDs,   
		    @MemberTypeIDs,   
...
ELSE IF @ResultOption = 2  
    BEGIN  
	    EXEC mdm.udpMetadataAttributeGetDetailsXML  
		    @User_ID,   
		    @ModelIDs,   
		    @EntityIDs,   
		    @MemberTypeIDs,   
...
SELECT @Result     
   FOR XML PATH(''), ELEMENTS XSINIL, ROOT('ArrayOfMetadataAttribute')  
