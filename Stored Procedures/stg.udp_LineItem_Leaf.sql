...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_LineItem_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[LineItem_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[LineItem_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[LineItem_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[LineItem_Leaf] stgl  
    INNER JOIN mdm.[tbl_4_38_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Group]', inserted.[Group], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[LineItem_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_4_37_EN dm   
    ON stgl.[Group] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[Group],  @NULLText), N'')) > 0   
...
FROM [stg].[LineItem_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_4_38_EN] en   
...
FROM [stg].[LineItem_Leaf] sc  
    INNER JOIN mdm.[tbl_4_38_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[LineItem_Leaf] sc  
    INNER JOIN mdm.[tbl_4_38_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[LineItem_Leaf]  
...
(  
            ...
            ,uda_38_941 NVARCHAR(MAX) NULL   
            ,New_uda_38_941 NVARCHAR(MAX) NULL   
);  
...
,[uda_38_941] --[Group]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[Group] = @NULLText THEN NULL ELSE [Group].ID END  
    FROM [stg].[LineItem_Leaf] stgl   
LEFT JOIN mdm.tbl_4_37_EN [Group] ON stgl.[Group] = [Group].Code AND  [Group].Version_ID = @Version_ID  
...
,[uda_38_941] --[Group]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_38_941])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_4_38_EN] en INNER JOIN [stg].[LineItem_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[LineItem_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_4_38_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_38_941] = CASE WHEN stgl.[Group] = @NULLText THEN NULL   
            ELSE ISNULL([Group].ID, en.[uda_38_941]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_38_941]) , CONVERT(NVARCHAR(MAX), inserted.[uda_38_941])   
        INTO #TRANLOG  
    FROM mdm.[tbl_4_38_EN] en INNER JOIN [stg].[LineItem_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[LineItem_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[LineItem_Leaf]   
