...
AS BEGIN  
    ...
    EXEC udpEntityStagingDeleteStoredProcedures @Entity_ID, 4  
      
    SET @MainSQL = N'CREATE PROCEDURE [stg].[udp_' + @StagingBase + N'_Relationship]    
    ...
    SET @MainSQL = @MainSQL + N'  
        ...
        UPDATE [stg].' + @StagingRelationshipTable + N'  
            ...
            WHERE ChildCode IN  
                ...
                INNER JOIN mdm.' + @EntityTable + N' en ON stgr.ChildCode = en.Code WHERE en.Version_ID = @Version_ID AND en.Status_ID = 2)  
        ...
        UPDATE [stg].' + @StagingRelationshipTable + N'  
            ...
            WHERE ChildCode NOT IN  
                (SELECT DISTINCT stgr.ChildCode FROM [stg].' + @StagingRelationshipTable + N' stgr  
                INNER JOIN mdm.' + @EntityTable + N' en ON stgr.ChildCode = en.Code WHERE en.Version_ID = @Version_ID)  
        ...
        UPDATE [stg].' + @StagingRelationshipTable + N'  
            ...
            WHERE ParentCode IN  
                (SELECT DISTINCT stgr.ParentCode FROM [stg].' + @StagingRelationshipTable + N' stgr  
                INNER JOIN mdm.' + @EntityTable + N' en ON stgr.ParentCode = en.Code WHERE en.Version_ID = @Version_ID)  
                AND ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID AND RelationshipType = 1;  
