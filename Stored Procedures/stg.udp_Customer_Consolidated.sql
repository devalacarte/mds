...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_Customer_Consolidated]    
...
BEGIN    
    ...
    IF LEN(@BatchTag) > 0 BEGIN  
        SELECT @MemberCount = COUNT(ID) FROM [stg].[Customer_Consolidated]   
    ...
    IF LEN(@BatchTag) > 0 BEGIN  
        IF EXISTS (SELECT stgp.ID FROM [stg].[Customer_Consolidated] stgp  
    ...
    -- Create a new batch ID.  
        ...
        IF @GetNewBatch_ID = @BatchIDNotFound BEGIN  
            UPDATE [stg].[Customer_Consolidated]  
    ...
    UPDATE [stg].[Customer_Consolidated]  
        ...
        WHERE Code in (SELECT Code FROM [stg].[Customer_Consolidated] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
    ...
    FROM [stg].[Customer_Consolidated] stgp  
    INNER JOIN mdm.[tbl_2_12_HP] hp   
    ...
    FROM [stg].[Customer_Consolidated] stgp  
        LEFT OUTER JOIN mdm.[tbl_2_12_HP] hp   
    ...
    UPDATE [stg].[Customer_Consolidated]  
        ...
        FROM [stg].[Customer_Consolidated] sc  
    ...
    BEGIN              
        ...
        SELECT  
            ...
            ON stgp.HierarchyName = hr.Name AND hr.Entity_ID = 12   
    ...
    SELECT DISTINCT ID AS Hierarchy_ID, [Name] AS Hierarchy_Name FROM mdm.tblHierarchy WHERE Entity_ID = 12   
        ...
        -- Add the member to the root of the hierarchy.   
            ...
            BEGIN  
                ...
                SELECT     
                    ...
                    GETUTCDATE(),     
                    @User_ID    
                FROM mdm.[tbl_2_12_HP] hp INNER JOIN [stg].[Customer_Consolidated] stgp   
        ...
        UPDATE [stg].[Customer_Consolidated]   
            SET ImportStatus_ID = @StatusProcessing  
            WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_2_12_HP] WHERE Version_ID = @Version_ID)   
        ...
        UPDATE [stg].[Customer_Consolidated]   
            SET ImportStatus_ID = @StatusOK  
            FROM [stg].[Customer_Consolidated]  
        ...
        IF Exists(SELECT Code FROM [stg].[Customer_Consolidated]   
    ...
    --Get the number of errors for the batch ID  
        SELECT @ErrorCount = COUNT(ID) FROM [stg].[Customer_Consolidated]  
