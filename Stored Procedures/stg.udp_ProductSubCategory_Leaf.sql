...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_ProductSubCategory_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[ProductSubCategory_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[ProductSubCategory_Leaf] stgl  
...
-- Create a new batch ID.  
    ...
    IF @GetNewBatch_ID = @BatchIDNotFound BEGIN  
        UPDATE [stg].[ProductSubCategory_Leaf]  
...
WHERE Code in (SELECT Code FROM [stg].[ProductSubCategory_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[ProductSubCategory_Leaf] stgl  
    INNER JOIN mdm.[tbl_3_28_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[ProductCategory]', inserted.[ProductCategory], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[ProductSubCategory_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_3_25_EN dm   
    ON stgl.[ProductCategory] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[ProductCategory],  @NULLText), N'')) > 0   
...
FROM [stg].[ProductSubCategory_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_3_28_EN] en   
...
FROM [stg].[ProductSubCategory_Leaf] sc  
    INNER JOIN mdm.[tbl_3_28_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[ProductSubCategory_Leaf] sc  
    INNER JOIN mdm.[tbl_3_28_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[ProductSubCategory_Leaf]  
...
(  
            ...
            ,uda_28_716 NVARCHAR(MAX) NULL   
            ,New_uda_28_716 NVARCHAR(MAX) NULL   
);  
...
,[uda_28_716] --[ProductCategory]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[ProductCategory] = @NULLText THEN NULL ELSE [ProductCategory].ID END  
    FROM [stg].[ProductSubCategory_Leaf] stgl   
LEFT JOIN mdm.tbl_3_25_EN [ProductCategory] ON stgl.[ProductCategory] = [ProductCategory].Code AND  [ProductCategory].Version_ID = @Version_ID  
...
,[uda_28_716] --[ProductCategory]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_28_716])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_3_28_EN] en INNER JOIN [stg].[ProductSubCategory_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[ProductSubCategory_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_3_28_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_28_716] = CASE WHEN stgl.[ProductCategory] = @NULLText THEN NULL   
            ELSE ISNULL([ProductCategory].ID, en.[uda_28_716]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_28_716]) , CONVERT(NVARCHAR(MAX), inserted.[uda_28_716])   
        INTO #TRANLOG  
    FROM mdm.[tbl_3_28_EN] en INNER JOIN [stg].[ProductSubCategory_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[ProductSubCategory_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[ProductSubCategory_Leaf]   
