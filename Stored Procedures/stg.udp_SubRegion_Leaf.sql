...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_SubRegion_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[SubRegion_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[SubRegion_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[SubRegion_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[SubRegion_Leaf] stgl  
    INNER JOIN mdm.[tbl_2_19_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[Region]', inserted.[Region], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[SubRegion_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_14_EN dm   
    ON stgl.[Region] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[Region],  @NULLText), N'')) > 0   
...
FROM [stg].[SubRegion_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_2_19_EN] en   
...
FROM [stg].[SubRegion_Leaf] sc  
    INNER JOIN mdm.[tbl_2_19_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[SubRegion_Leaf] sc  
    INNER JOIN mdm.[tbl_2_19_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[SubRegion_Leaf]  
...
(  
            ...
            ,uda_19_408 NVARCHAR(MAX) NULL   
            ,New_uda_19_408 NVARCHAR(MAX) NULL   
);  
...
,[uda_19_408] --[Region]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[Region] = @NULLText THEN NULL ELSE [Region].ID END  
    FROM [stg].[SubRegion_Leaf] stgl   
LEFT JOIN mdm.tbl_2_14_EN [Region] ON stgl.[Region] = [Region].Code AND  [Region].Version_ID = @Version_ID  
...
,[uda_19_408] --[Region]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_19_408])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_2_19_EN] en INNER JOIN [stg].[SubRegion_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[SubRegion_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_2_19_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_19_408] = CASE WHEN stgl.[Region] = @NULLText THEN NULL   
            ELSE ISNULL([Region].ID, en.[uda_19_408]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_19_408]) , CONVERT(NVARCHAR(MAX), inserted.[uda_19_408])   
        INTO #TRANLOG  
    FROM mdm.[tbl_2_19_EN] en INNER JOIN [stg].[SubRegion_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[SubRegion_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[SubRegion_Leaf]   
