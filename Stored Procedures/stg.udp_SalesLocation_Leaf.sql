...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [stg].[udp_SalesLocation_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    SELECT @MemberCount = COUNT(ID) FROM [stg].[SalesLocation_Leaf]  
...
IF LEN(@BatchTag) > 0 BEGIN  
    IF EXISTS (SELECT stgl.ID FROM [stg].[SalesLocation_Leaf] stgl  
...
WHERE Code in (SELECT Code FROM [stg].[SalesLocation_Leaf] WHERE ImportStatus_ID = @StatusDefault AND Batch_ID = @Batch_ID group by Code having COUNT(*) > 1);  
...
FROM [stg].[SalesLocation_Leaf] stgl  
    INNER JOIN mdm.[tbl_2_16_EN] en   
...
SET ErrorCode = IsNull(ErrorCode,0) | 4  
    OUTPUT inserted.Batch_ID, inserted.Code, N'[SubRegion]', inserted.[SubRegion], 210003  
    INTO [mdm].[tblStgErrorDetail] (Batch_ID, Code, AttributeName, AttributeValue, UniqueErrorCode)  
FROM [stg].[SalesLocation_Leaf] stgl  
    LEFT OUTER JOIN mdm.tbl_2_19_EN dm   
    ON stgl.[SubRegion] = dm.Code  
...
AND LEN(COALESCE(NULLIF(stgl.[SubRegion],  @NULLText), N'')) > 0   
...
FROM [stg].[SalesLocation_Leaf] stgl  
    LEFT OUTER JOIN mdm.[tbl_2_16_EN] en   
...
FROM [stg].[SalesLocation_Leaf] sc  
    INNER JOIN mdm.[tbl_2_16_EN] AS tSource ON sc.Code = tSource.Code  
...
SET ErrorCode = IsNull(ErrorCode,0) | 512  
FROM [stg].[SalesLocation_Leaf] sc  
    INNER JOIN mdm.[tbl_2_16_EN] AS tSource ON sc.NewCode = tSource.Code  
...
BEGIN  
    --Generate the codes.                         
    SELECT @NumberOfCodes = COUNT(*) FROM [stg].[SalesLocation_Leaf]  
...
(  
            ...
            ,uda_16_407 NVARCHAR(MAX) NULL   
            ,New_uda_16_407 NVARCHAR(MAX) NULL   
);  
...
,[uda_16_407] --[SubRegion]  
    ...
    SELECT  
        ...
        GETUTCDATE(),  
        @User_ID,  
        @Version_ID,CASE WHEN stgl.[SubRegion] = @NULLText THEN NULL ELSE [SubRegion].ID END  
    FROM [stg].[SalesLocation_Leaf] stgl   
LEFT JOIN mdm.tbl_2_19_EN [SubRegion] ON stgl.[SubRegion] = [SubRegion].Code AND  [SubRegion].Version_ID = @Version_ID  
...
,[uda_16_407] --[SubRegion]  
    ...
    OUTPUT inserted.ID, inserted.Code, inserted.Code, inserted.Name, inserted.Name , NULL , CONVERT(NVARCHAR(MAX), inserted.[uda_16_407])   
        INTO #TRANLOG  
    SELECT  
        @Version_ID,  
        1,  
...
BEGIN  
    ...
    SELECT     
        ...
        GETUTCDATE(),     
        @User_ID    
    FROM mdm.[tbl_2_16_EN] en INNER JOIN [stg].[SalesLocation_Leaf] stgl   
        ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
UPDATE [stg].[SalesLocation_Leaf]  
SET ImportStatus_ID = @StatusProcessing  
WHERE ImportType in (@IT_MergeOptimistic, @IT_MergeOverwrite) AND Code in (SELECT Code FROM mdm.[tbl_2_16_EN] WHERE Version_ID = @Version_ID)  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        ,[uda_16_407] = CASE WHEN stgl.[SubRegion] = @NULLText THEN NULL   
            ELSE ISNULL([SubRegion].ID, en.[uda_16_407]) END  
...
BEGIN  
    ...
    SET ValidationStatus_ID = @AwaitingRevalidation  
        ...
        OUTPUT inserted.ID, deleted.Code, inserted.Code, deleted.Name, inserted.Name , CONVERT(NVARCHAR(MAX), deleted.[uda_16_407]) , CONVERT(NVARCHAR(MAX), inserted.[uda_16_407])   
        INTO #TRANLOG  
    FROM mdm.[tbl_2_16_EN] en INNER JOIN [stg].[SalesLocation_Leaf] stgl   
    ON en.Code = stgl.Code AND en.Version_ID = @Version_ID  
...
-- Check circular references only when there is a DBA to check.  
    ...
    IF @RecursiveHierarchy_ID > 0 BEGIN  
        ...
        BEGIN  
            ...
            SELECT @ErrorCount = COUNT(ID) FROM [stg].[SalesLocation_Leaf]  
                WHERE Batch_ID = @Batch_ID AND ImportStatus_ID = @StatusError;  
...
IF Exists(SELECT Code FROM [stg].[SalesLocation_Leaf]   
